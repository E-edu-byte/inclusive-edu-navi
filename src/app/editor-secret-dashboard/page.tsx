...
// Daily English Snap 用 Supabase クライアント
const englishSupabaseUrl = process.env.NEXT_PUBLIC_ENGLISH_SUPABASE_URL || '';
const englishSupabaseKey = process.env.NEXT_PUBLIC_ENGLISH_SUPABASE_ANON_KEY || '';
const englishSupabase = englishSupabaseUrl && englishSupabaseKey
  ? createClient(englishSupabaseUrl, englishSupabaseKey)
  : null;

// タブの種類
type TabType = 'inclusive' | 'english';

// Daily English Snap のフレーズ型
type EnglishPhrase = {
  id: string;
  phrase: string;
  meaning: string;
...
export default function EditorDashboard() {
  // タブ状態
  const [activeTab, setActiveTab] = useState<TabType>('inclusive');
  ...
  useEffect(() => {
    ...
    // Daily English Snap のフレーズを取得
    async function fetchEnglishPhrases() {
      if (!englishSupabase) {
      ...
      try {
        ...
        if (data && !error) {
          setEnglishPhrases(data);
        }
      } catch (e) {
        console.error('English phrases fetch error:', e);
      } finally {
        setEnglishLoading(false);
      }
    }
  ...
  // Daily English Snap フレーズ削除
  const handleDeleteEnglishPhrase = async (phraseId: string) => {
    if (!englishSupabase) return;
    if (!confirm('このフレーズを削除しますか？')) return;

    setDeletingPhraseId(phraseId);
    try {
      const { error } = await englishSupabase
        .from('phrases')
        .delete()
        .eq('id', phraseId);
      ...
      if (error) {
        ...
        setEnglishPhrases((prev) => prev.filter((p) => p.id !== phraseId));
      }
    } catch (e) {
      alert(`例外: ${e instanceof Error ? e.message : '不明なエラー'}`);
    } finally {
          ...
          <>
            ...
            <section className="bg-gray-800 rounded-xl p-6 mb-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                <div className="bg-gray-700 rounded-lg p-4">
                  <span className="text-gray-400 text-xs block mb-1">総フレーズ数</span>
                  <span className="text-white font-medium text-lg">{englishPhrases.length}件</span>
                </div>
                <div className="bg-gray-700 rounded-lg p-4">
                  <span className="text-gray-400 text-xs block mb-1">最終更新</span>
                  <span className="text-white font-medium text-sm">
                    {englishPhrases[0]?.created_at ? formatDate(englishPhrases[0].created_at) : '-'}
                  </span>
                </div>
                <div className="bg-gray-700 rounded-lg p-4">
                  <span className="text-gray-400 text-xs block mb-1">サイトURL</span>
                  <a
                    href="https://english.news-navi.jp"
                    target="_blank"
                    rel="noopener noreferrer"
            ...
            <section className="bg-gray-800 rounded-xl p-6 mb-6">
              ...
              <div className="bg-gray-700 rounded-lg p-4">
                <p className="text-gray-300 text-sm mb-4">
                  GitHub Actions を使って新しいフレーズを即座に生成できます。
                </p>
                <a
                  href="https://github.com/E-edu-byte/daily-english-snap/actions/workflows/generate.yml"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 px-4 py-2 bg-yellow-500 hover:bg-yellow-600 text-stone-900 font-medium rounded-lg transition-colors"
                >
            ...
            <section className="bg-gray-800 rounded-xl p-6 mb-6">
              <h2 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
                ...
                <span className="ml-2 px-2 py-0.5 text-xs bg-yellow-600 text-white rounded-full">
                  {englishPhrases.length}件
                </span>
              </h2>

              {!englishSupabase ? (
                <div className="bg-gray-700 rounded-lg p-6 text-center">
                  <p className="text-gray-400 mb-2">Supabase 接続が設定されていません</p>
                  <p className="text-xs text-gray-500">
                    環境変数 NEXT_PUBLIC_ENGLISH_SUPABASE_URL と<br />
                    NEXT_PUBLIC_ENGLISH_SUPABASE_ANON_KEY を設定してください
                  </p>
                </div>
              ) : englishLoading ? (
                <div className="text-center py-6">
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-lg font-medium text-yellow-400 mb-1">
                            {phrase.phrase}
                          </p>
                          <p className="text-sm text-gray-300 mb-2">{phrase.meaning}</p>
                          <p className="text-xs text-gray-500 line-clamp-2">{phrase.nuance}</p>
                          <div className="mt-2 flex items-center gap-2">
                            <span className="px-2 py-0.5 text-xs bg-gray-600 text-gray-300 rounded">
                              穴埋め: {phrase.blank_word}
                            </span>
                          </div>
                        </div>
                        <button
                          onClick={() => handleDeleteEnglishPhrase(phrase.id)}
                          disabled={deletingPhraseId === phrase.id}
                          className="flex-shrink-0 p-2 text-red-400 hover:text-red-300 hover:bg-red-900/30 rounded-lg transition-colors disabled:opacity-50"
                          title="削除"
                        >
                          {deletingPhraseId === phrase.id ? (
                            <div className="w-5 h-5 border-2 border-red-400 border-t-transparent rounded-full animate-spin"></div>
                          ) : (
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                ...
                <p className="text-center text-gray-400 py-6">フレーズはまだありません</p>
              )}
            </section>
          </>
        )}
